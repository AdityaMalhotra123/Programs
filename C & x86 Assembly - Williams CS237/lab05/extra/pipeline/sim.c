      ...
      for (int j = 0; j < numDestRegisters; j++) {
       for (int k = 0; k < numSrcRegisters; k++) {
         if (destRegisters[j] == srcRegisters[k]) {
          // If instruction in stage from which calculated data is forwarded 
          // requires memory access, check whether that instruction has
          // crossed the stage after memory access. (Need to stall till data
          // is forwarded before it is used by current instruction.)
          if (getSourceOperandType(pipeline[i]->name, 0) == OP_MEMORY) {
            ...
            // If instruction in stage from which calculated data is forwarded 
            // doesn't require memory access, check whether that instruction has
            // crossed the stage after calculation.
					       ...
					       int numInstrs)
{
  ...
  while(enteredCount < numInstrs || !isPipelineEmpty(pipeline)){
    ...
    // If the pipeline stage is the stage where we read forwarded
    // data due to an instruction in another stage, stall if data
