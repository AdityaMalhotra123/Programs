 ...
 *
 * Terminology:
 * - We will implement an explicit free list allocator
 * - We use "next" and "previous" to refer to blocks as ordered in
 *   the free list.
 * - We use "following" and "preceding" to refer to adjacent blocks
 *   in memory.
 *-------------------------------------------------------------------- */
...
// Given that a block of size size needs to be allocated,
// find the actual memory size.
size_t getReqSize(size_t size) {
...
// Given a free memory block specified by ptrFreeBlock, which is
// of size >= reqSize + MIN_BLOCK_SIZE, split it into two free blocks, one of which
// is of size reqSize, and update the free list accordingly.
void splitFreeBlock(BlockInfo* ptrFreeBlock, size_t reqSize) {
  // ptrSplitFreeBlock is the extra block that results from splitting.
  BlockInfo* ptrSplitFreeBlock = NULL;

  // Record some data about the block.
  size_t blockSize = SIZE(ptrFreeBlock->sizeAndTags);
  size_t precedingBlockUseTag = ptrFreeBlock->sizeAndTags & TAG_PRECEDING_USED;

  // Update the header of the initial block and add a footer at a new location,
  // which effectively makes it into a smaller block.
  ptrFreeBlock->sizeAndTags = reqSize | precedingBlockUseTag;
  *((size_t*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize - WORD_SIZE)) =
        ptrFreeBlock->sizeAndTags;

  // Place the header and the footer for the new block that results from splitting.
  // The "used" and "preceding used" bits will both be 0, so the header and
  // footer are just the size.
  ptrSplitFreeBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock, reqSize);
  ptrSplitFreeBlock->sizeAndTags = (blockSize - reqSize);
  *((size_t*)UNSCALED_POINTER_ADD(ptrSplitFreeBlock,
		 (blockSize - reqSize) - WORD_SIZE)) = (blockSize - reqSize);

  // Insert the newly formed "splinter" block into the free list.
  insertFreeBlock(ptrSplitFreeBlock);
}

//Updates allocated status of current free block and tags of next block.
void updateFreeLists(BlockInfo* ptrFreeBlock) {
  BlockInfo* ptrNextBlock = NULL;
  // Update the "preceding used" tag for the following block.
  ptrNextBlock = (BlockInfo*)UNSCALED_POINTER_ADD(ptrFreeBlock,
						  SIZE(ptrFreeBlock->sizeAndTags));
  ptrNextBlock->sizeAndTags = ptrNextBlock->sizeAndTags | TAG_PRECEDING_USED;
  *((size_t*)UNSCALED_POINTER_ADD(ptrNextBlock,
	SIZE(ptrNextBlock->sizeAndTags) - WORD_SIZE)) = ptrNextBlock->sizeAndTags;

  // Update the used tag for this block.
  // Cut out the block from the explicit list of free blocks.
...
void* mm_malloc (size_t size) {
  ...
  // Compute the amount of memory required to host the block to be allocated.
  reqSize = getReqSize(size);

  // Look for a sufficiently large free block; if not found, request more memory.
  ptrFreeBlock = searchFreeList(reqSize);
  if(ptrFreeBlock == NULL) {
    requestMoreSpace(8 * reqSize);  // The number 8 is arbitrary.

    ptrFreeBlock = searchFreeList(reqSize);
  }
  
  // If the block that was found is significantly larger than the required size,
  // split the block in two.
  if (SIZE(ptrFreeBlock->sizeAndTags) >= reqSize + MIN_BLOCK_SIZE) {
    splitFreeBlock(ptrFreeBlock, reqSize);
  }
  
  updateFreeLists(ptrFreeBlock);

  // Return the pointer to the first "payload" byte of the allocated memory block.
  //return ptrFreeBlock;
...
void mm_free (void *ptr) {
  ...
  // Retrieve a pointer to the metadata for our block.
  blockInfo = (BlockInfo*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  payloadSize = SIZE(blockInfo->sizeAndTags) - (2 * WORD_SIZE);

  // Set the used tag in the header and footer of our block to zero.
  blockInfo->sizeAndTags = blockInfo->sizeAndTags & (~TAG_USED);
  *((size_t*)UNSCALED_POINTER_ADD(blockInfo, payloadSize + WORD_SIZE)) =
          blockInfo->sizeAndTags;

  // Set the preceding used tag in the header and footer of the following
  // block to zero.
  followingBlock = (BlockInfo*)UNSCALED_POINTER_ADD(blockInfo,
						    SIZE(blockInfo->sizeAndTags));
  followingBlock->sizeAndTags = followingBlock->sizeAndTags & (~TAG_PRECEDING_USED);
  *((size_t*)UNSCALED_POINTER_ADD(followingBlock,
      SIZE(followingBlock->sizeAndTags) - WORD_SIZE)) = followingBlock->sizeAndTags;

  // Insert the newly freed block into the free list and coalesce.
  insertFreeBlock(blockInfo);
  coalesceFreeBlock(blockInfo);
}


// Implement a heap consistency checker as needed (helpful, but not required)
int mm_check() {
  return 0;
}


// The min function for size_t numbers
...
void* mm_realloc(void* ptr, size_t size) {
  ...
  if (ptr == NULL) {
    ...
    //for(int i = 0; i < min(oldSize, size) / WORD_SIZE; i++) {
    for (int i = 0; i < min(oldSize, size); i++) {
      //*((size_t*)UNSCALED_POINTER_ADD(newPtr, i)) =
      //            *((size_t*)UNSCALED_POINTER_ADD(ptr, i));
      *((char*)UNSCALED_POINTER_ADD(newPtr, i)) =
	            *((char*)UNSCALED_POINTER_ADD(ptr, i));
    }
    mm_free(ptr);
